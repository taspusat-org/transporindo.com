...
import { useMemo, useState, useEffect, Suspense } from "react"; // Added Suspense
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`transition-all duration-300 pb-1 border-b-2 ${isActive ? "text-orange-500 border-orange-500" : "border-transparent hover:text-orange-500 hover:border-orange-500"}`}
              >
                {link.label}
              </Link>
            );
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={(e) => handleScroll(e, link.isExternalPage)}
                  className={`text-xl font-bold transition-colors ${link.isExternalPage ? "bg-orange-100 text-orange-600 px-10 py-3 rounded-full mt-4" : isActive ? "text-orange-500" : "hover:text-orange-500"}`}
                >
                  {link.label}
                </Link>
              );
...
export default function NavbarForError() {
  return (
    <Suspense fallback={<div className="h-20 bg-white w-full fixed top-0" />}>
      <NavbarContent />
    </Suspense>
  );
}
